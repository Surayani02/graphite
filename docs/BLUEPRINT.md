...
| `packages/engine`         | Rust scene graph (SDF shapes + tessellated paths), compiled to WASM                                                              | Active                                                                                                                            |
| `packages/geometry`       | Path geometry: §D.1 types, lyon fill/stroke tessellation, cull-grade bounds — internal crate the engine re-exports (ADR-031/032) | Active — Phase 8 M1                                                                                                               |
| `packages/document`       | Placeholder — see [ADR-010](./adr/ADR-010-document-crate-status.md)                                                              | Inert                                                                                                                             |
| `packages/document-model` | Document model: `DocumentModel`, ops, validation, tree derivation, `.graphite` format                                            | Active — extracted from `apps/web` at Phase 8 PC-1 (ADR-030)                                                                      |
